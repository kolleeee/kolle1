...
class   top_1(object):
    ...
    #添加数据
    def tableView_add(self,ints,s1,s2,s3):  #添加数据
        try:
            #红色：(255,0,0)
            #绿色：(0,255,0)
            if not s1==None:
            ...
            if s2=="No" or s2=="null" or s2==None:  #红色  #改变背景色
        ...
        if isfile(self.saveHistoryFilename):
            ...
            for xxx_line in xxx.readlines():
                try:
                    ...
                    data2=u"%s"%(data)
                    self.tableView_add(i,data2 ,None,data_time2)  #添加数据
                    i+=1
                except BaseException, e:
                    print(str(e))
    ...
    def SQL_Button_1(self): #全部显示
        ...
        for line in res:
            ...
            self.tableView_add(int_id,s0,str(line[1]),self.time_localtime(line[2]))  #添加数据
            int_id+=1
        cur.close()  #关闭游标
    ...
    def thread_RE_Button(self,re_data):
        ...
        for index in int_model.selectedRows():       #// 对于被选中的每一行
            try:
                ...
                A_BOOL,A_DATA=self.re_cx_data(re_data,str(s0))  #正则查询内容是否存在
                if A_BOOL:
                    #print A_DATA
                    if self.url_http_200(A_DATA):
                        sql_data="update url set bool='ok' where url='%s'"%(urllib.quote(str(s0)))
                        #print sql_data
                        self.sql3.mysqlite3_update(sql_data)
                        self.tableView_add(int_index,None,"ok",None)  #添加数据
                    else:
                        sql_data="update url set bool='No' where url='%s'"%(urllib.quote(str(s0)))
                        #print sql_data
                        self.sql3.mysqlite3_update(sql_data)
                        self.tableView_add(int_index,None,"No",None)  #添加数据
            except BaseException, e:
                print(str(e))
    ...
    def pushButton3(self): #删除数据
        ...
        for index in int_model.selectedRows():       #// 对于被选中的每一行
            try:
                int_index=index.row()#获取行号
                s0= model.data(model.index(int_index,0)).toString()
                sql_data="delete from url where url='%s'"%(urllib.quote(str(s0)))
    ...
    def pushButton(self): #生成链接
        ...
        data_1=self.ui.textEdit.toPlainText() #获取内容
        data_2=self.ui.textEdit_2.toPlainText() #获取内容
