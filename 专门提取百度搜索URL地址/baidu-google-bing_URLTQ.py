...
#            self.bing_TQ_URL("http://cn.bing.com/search?q=inurl:admin/user.asp&go=&first=10") #提取bing  URL
    ...
    def for_url(self,data):  #循环查找
        ...
        try:
            while self.a<=b:
                url_data="http://www.baidu.com/s?wd=%s&pn=%d&rn=50&usm=1"%(data,self.a)
    ...
    def baidu_KZ_URL(self,URL): #提取百度快照  URL
        try:
            ...
            if ss==0:  #读取网页内容
                ...
                return 0
            print u"当前URL: %s"%(URL)
            #<span class="g">www.macgg.com/archives/82...html&nbsp;2012-09-10&nbsp;</span>
            p = re.compile( r'<span class="g">(.+?)&nbsp')
...
##            urls = parser.urls
##            list_2=[]
##            for i in urls:  #去重重复数据
##                if i not in list_2:
##                    list_2.append(i)
##            for urldata in list_2:
##                print urldata
##                a1=self.URL_TQURL(urldata)
##                print a1
##                skl=0
##                for A in [";","&","="]:
##                    if str(a1).__contains__(A):
