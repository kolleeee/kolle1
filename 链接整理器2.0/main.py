...
友情链接："""
        ...
        self.ui.textEdit_4.insertPlainText(u"%s"%(data2))
        data3="""</div>"""
        self.ui.textEdit_2.insertPlainText(u"%s"%(data3))
        #事件处理
        ##########################
    ...
    def ini(self): #初始化
        ...
        self.model.setHorizontalHeaderLabels([u'链接地址',u'关键字',u'状态',u'最新操作时间'])
        self.ui.SQLite_tableView.setModel(self.model)
        #self.tableView.resizeColumnsToContents()   #由内容调整列
        self.ui.SQLite_tableView.setColumnWidth(0,450)  #设置表格的各列的宽度值
        self.ui.SQLite_tableView.setColumnWidth(1,150)  #设置表格的各列的宽度值
        self.ui.SQLite_tableView.setColumnWidth(2,50)  #设置表格的各列的宽度值
        self.ui.SQLite_tableView.setColumnWidth(3,170)  #设置表格的各列的宽度值
